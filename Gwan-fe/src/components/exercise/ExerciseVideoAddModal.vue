<template>
    <div class="modal-backdrop">
      <transition name="modal-fade">
        <div v-if="visible" class="modal-container">
          <h2 class="modal-title">운동 영상 추가</h2>
          <form @submit.prevent="handleSubmit">
            ...
            <div class="modal-actions">
              <button type="submit" class="btn btn-primary w-50" :disabled="loading">등록</button>
  ...
  const error = ref('')
  const targets = ref([])
  const visible = ref(false) // 애니메이션용
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      error.value = err.response?.data?.message || '등록 실패'
    } finally {
      loading.value = false
    }
  }
